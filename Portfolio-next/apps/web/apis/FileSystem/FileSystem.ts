...
export function createBaseFileSystem(): FileSystem {
  ...
  const hayley = fileSystem.addDirectory(users, 'hayley', false, true);
...
function entriesWithinSelection(entries: Point[], x: number, y: number, dimensions: { width: number, height: number }): number {
  ...
  // NOTE(Hayley): The width & height are used for both the new incoming entry as the already existing entries.
