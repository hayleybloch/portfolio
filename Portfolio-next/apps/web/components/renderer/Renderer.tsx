...
import { DepthTexture, LinearFilter, PerspectiveCamera, RGBAFormat, Scene, VSMShadowMap, WebGLRenderer, WebGLRenderTarget, SRGBColorSpace } from "three";
...
// Dynamically import Stats in-effect to avoid SSR issues
let StatsModule: any = null;
...
export const Renderer = (props: RendererProps) => {
  ...
  const cssOutputRef = useRef<HTMLDivElement | null>(null);
  const webglOutputRef = useRef<HTMLDivElement | null>(null);
  ...
  const then = useRef<number | null>(null);
  ...
  useEffect(() => {
    ...
    if (!scenes || !scenes.sourceScene || !scenes.cutoutScene || !scenes.cssScene) {
      return;
    }
    ...
    if (debug) {
      // Lazy-load Stats only in the browser
      (async () => {
        try {
          if (!StatsModule) {
            StatsModule = await import("three/examples/jsm/libs/stats.module");
          }
    ...
    // Ensure correct color space for GLTF baseColor/albedo textures
    // (Three r152+): outputColorSpace is preferred over outputEncoding
    // @ts-ignore
    ...
    saoPass.params.saoIntensity = 0.0006; // subtle boost for depth
  ...
  useEffect(() => {
    if (!loading && cameraHandlerRef.current) {
